...
def evaluate(model,data_loader,best_wa,history_was):
    ...
    for sample_speech,sample_sentence,sample_labels in data_loader:
        ...
        predict = prob.cpu().detach()#分离tensor
...
for epoch in range(hp.epochs):
    ...
    for sample_speech,sample_text,sample_label in tqdm(train_loader):
        ...
        speech_prob, text_prob, prob = model(sample_speech,sample_text)
        loss = loss_function(prob, sample_label.view(-1))
        loss += loss_function(speech_prob, sample_label.view(-1))
        loss += loss_function(text_prob, sample_label.view(-1))
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(model.parameters(), hp.grad_clip)  # 梯度裁剪
        optimizer.step()
